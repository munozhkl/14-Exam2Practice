...
class Box(object):
    ...
    def append_string(self, additional_contents):
        ...
        if len(additional_contents) <= space:
            number_of_characters_to_append = len(additional_contents)
        else:
        ...
        stuff_to_return = ''
        for k in range(len(additional_contents) - len(stuff_to_add)):
            stuff_to_return = stuff_to_return + additional_contents[k + number_of_characters_to_append]
    ...
    def shrink(self, new_volume):
        ...
        if self.volume <= len(self.contents):
            for k in range(new_volume, len(self.contents)):
                discard = discard + self.contents[k]
            for k in range(new_volume):
                new_content = new_content + self.contents[k]
            if self.contents == new_content:
                return ''
    ...
    def reset(self):
        ...
        #    TIME ESTIMATE:   5 minutes.
        # --------------------------------------------------------------
        before_reset = self.contents
        self.history = self.history + [before_reset]
        self.volume = self.original_volume
        self.contents = self.original_contents
        self.num_reset_called = self.num_reset_called + 1
